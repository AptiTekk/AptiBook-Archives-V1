 ...
 * most people.
 * <p>
 * An IP ban will not work as it would end up banning the entire school or business.
 */
public class BanHelper {
    ...
    public static void recordFailedAttempt(String banName) {
        int failedAttempts = FacesSessionHelper.getSessionVariableAsInt(PREFIX + banName);
        if (failedAttempts == -1)
            FacesSessionHelper.setSessionVariable(PREFIX + banName, 1);
        else
            FacesSessionHelper.setSessionVariable(PREFIX + banName, ++failedAttempts);
    }
    ...
    public static void clearFailedAttempts(String banName) {
        ...
        if (failedAttempts == -1)
            ...
            FacesSessionHelper.setSessionVariable(PREFIX + banName, 0);
    }
